...
export default function VisitorProfileClient({ username }: { username: string }) {
  ...
  const normalizedRouteUsername = decodeURIComponent(username).trim().toLowerCase();
  ...
  useEffect(() => {
    fetch("/api/user/me")
      ...
      .then((d) => {
        ...
        if (
          currentUsername &&
          currentUsername.trim().toLowerCase() === normalizedRouteUsername
        ) {
  ...
  }, [normalizedRouteUsername, router]);

  useEffect(() => {
    if (!selfCheckResolved || isOwnProfile) return;
    ...
    fetch(`/api/user/profile/${username}/friends`)
      ...
      .catch(() => {});
  }, [isOwnProfile, selfCheckResolved, username]);

  useEffect(() => {
    if (!selfCheckResolved || !profile || currentUserId === null) return;

    if (String(currentUserId) === String(profile.id)) {
      router.replace("/dashboard/profile");
      return;
    }
    ...
    fetch("/api/user/friends/requests")
      ...
      .catch(() => {});
  }, [currentUserId, profile, router, selfCheckResolved, username]);
  ...
  }, [isOwnProfile, selfCheckResolved, username, libraryFilter]);
  ...
  if (!selfCheckResolved || !profile) {
    return <div className="flex items-center justify-center min-h-screen text-white/40 text-sm">Loading…</div>;
  }

  if (isOwnProfile) {
    return <div className="flex items-center justify-center min-h-screen text-white/40 text-sm">Redirecting...</div>;
  }
