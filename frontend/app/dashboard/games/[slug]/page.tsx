...
export default function GameDetailsPage() {
  ...
  const handleAdd = async () => {
    ...
    else toastSuccess(`Added to ${selectedStatus}!`);
    setAdding(false);
  };
    ...
    <div className="p-6 md:p-10 text-white max-w-5xl mx-auto">
      <div className="relative w-full h-72 md:h-96 rounded-xl overflow-hidden mb-8">
        ...
        <div className="absolute bottom-6 left-6 right-6">
          ...
          {game.released && <p className="text-white/60 text-sm mt-1">{game.released}</p>}
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-8">
        ...
        >
          {STATUS_OPTIONS.map((s) => (
            <option key={s} value={s} className="bg-[rgb(20,20,35)] capitalize">{s}</option>
          ))}
        </select>
        <button
          onClick={handleAdd}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        {game.metacritic && <Stat label="Metacritic" value={game.metacritic} />}
        {game.price && <Stat label="Price" value={game.price} />}
        {game.developers?.[0] && <Stat label="Developer" value={game.developers[0]} />}
        {game.publishers?.[0] && <Stat label="Publisher" value={game.publishers[0]} />}
      </div>

      {game.description_raw && <p className="text-white/70 leading-relaxed mb-8">{game.description_raw}</p>}
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {game.genres.map((g: any) => (
              <span key={g.id} className="px-3 py-1 rounded-full bg-white/10 border border-white/10 text-sm text-white/70">{g.name}</span>
            ))}
          </div>
        </div>
      )}
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {game.platforms.map((p: any, i: number) => (
              <span key={i} className="px-3 py-1 rounded-full bg-white/10 border border-white/10 text-sm text-white/70">{p.platform?.name}</span>
            ))}
          </div>
        </div>
      )}
